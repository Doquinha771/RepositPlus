...
from reposit.backend.discovery import DiscoveryService
...
def configure_logging() -> None:
    ...
    handlers: list[logging.Handler] = [logging.FileHandler(PATHS.logs / "reposit.log", encoding="utf-8")]
    if not packaged:
...
def enable_windows_energy_saver() -> None:
...
class NativeBridge:
    ...
    def open_default_mail_settings(self) -> dict[str, Any]:
        try:
            if os.name == "nt":
    ...
    def exposed_functions(self) -> tuple[Any, ...]:
        ...
        return (
            self.open_file,
            self.open_default_mail_settings,
...
class DesktopRuntime:
    def __init__(self) -> None:
        ...
        self.discovery = DiscoveryService(self.db, self.identity, self.port, lambda: bool(self.settings.get("mural_enabled")))
    ...
    def run(self) -> None:
        ...
        # WebView2 needs a writable profile folder. Keeping it in AppData fixes
